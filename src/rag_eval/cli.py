...
def main() -> None:
    ...
    parser.add_argument(
        "--methods",
        help="Comma-separated methods: vector,pageindex,pageindex_official,rlm,rlm_pageindex (alias: rlm-pageindex).",
    )
...
def apply_overrides(cfg: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    if args.record_reasoning_trajectory:
        ...
        cfg["rlm_pageindex"]["record_reasoning_trajectory"] = True
    if args.no_record_reasoning_trajectory:
        ...
        cfg["rlm_pageindex"]["record_reasoning_trajectory"] = False
